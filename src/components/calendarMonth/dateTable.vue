        ...
        getCellClass({text, type}) {
            ...
            if (type === 'current') {
                ...
                if (date === moment().format('YYYY-MM-DD')) {
                    ...
                    date.slice(0, date.lastIndexOf('-')) === moment().format('YYYY-MM')
                    && Number(date.split('-')[2]) < Number(moment().format('DD'))) {
                    classes.push('is-past');
                }
                else if (date.split('-')[0] === moment().format('YYYY') && Number(mm) < Number(moment().format('MM'))) {
                    classes.push('is-past');
                }
            }
            else {
                classes.push('calendar-cantclick');
                let newSet = new Set(classes);
                newSet.delete('is-past');
                classes = [...newSet];
            }
